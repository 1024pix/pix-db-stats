...
module.exports = {
  async getDbMetrics(addonProvider) {
    const { addonId, token } = await _getScalingoDatabaseAPICredentials(addonProvider);

    return _getDbMetrics(token, addonId);
  },

  async getInstancesStatus(addonProvider) {
    const { addonId, token } = await _getScalingoDatabaseAPICredentials(addonProvider);
...
async function _getScalingoDatabaseAPICredentials(addonProvider) {
  const scalingoApplicationToken = await _getScalingoApplicationToken();
  const addonId = await _getAddonId(addonProvider, scalingoApplicationToken);
  const addonToken = await _getAddonToken(scalingoApplicationToken, addonId);
...
async function _getAddonId(addonProvider, token) {
  const config = requestConfig(token)
  const { data } = await httpService.get(`${SCALINGO_API_URL}/apps/${SCALINGO_APP}/addons`, config);
  const addon = data.addons.find(addon => addon['addon_provider'].id === addonProvider);
  return addon.id;
}
...
async function _getAddonToken(token, addonId) {
  const config = requestConfig(token)
