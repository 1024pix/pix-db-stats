...
const { SCALINGO_REGION, SCALINGO_TOKEN } = require('../../config');
...
module.exports = {
  ...
  async getPgRunningQueries(scalingoApp, injectedCredentials = _getScalingoDatabaseAPICredentials) {
  ...
  async resetPgStats(scalingoApp) {
    const { addonId, token } = await _getScalingoDatabaseAPICredentials(scalingoApp);
