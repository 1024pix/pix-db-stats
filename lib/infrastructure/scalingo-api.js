...
const { SCALINGO_REGION, SCALINGO_TOKEN, LONG_RUNNING_QUERY_DURATION_SECONDS, TOP_N_QUERIES_COUNT } = require('../../config');
...
module.exports = {
  ...
  async getRunningQueries(
    ...
    const longQueries = stats.result.filter(
      (query) => query.query_duration >= LONG_RUNNING_QUERY_DURATION_SECONDS * MICROSECONDS_IN_SECONDS
    );

    const topNQueries = stats.result.slice(0, TOP_N_QUERIES_COUNT);
