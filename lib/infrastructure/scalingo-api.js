...
const {
  SCALINGO_REGION,
  SCALINGO_TOKEN,
  LONG_RUNNING_QUERY_DURATION_SECONDS,
  TOP_N_QUERIES_COUNT,
} = require('../../config');
...
module.exports = {
  ...
  async getRunningQueries(scalingoApp, injectedCredentials = _getScalingoDatabaseAPICredentials) {
    ...
    const queries = stats.result;
    queries.sort((q1, q2) =>
      q1.query_duration < q2.query_duration ? 1 : q1.query_duration > q2.query_duration ? -1 : 0
    );

    const activeQueries = queries.filter((query) => query.state === 'active');

    const longQueries = activeQueries.filter(
      (query) => query.query_duration >= LONG_RUNNING_QUERY_DURATION_SECONDS * MICROSECONDS_IN_SECONDS
    );

    const NLongestQueries = activeQueries.slice(0, TOP_N_QUERIES_COUNT);

    return {
      queriesCount: activeQueries.length,
      longQueries,
      NLongestQueries,
    };
  },
